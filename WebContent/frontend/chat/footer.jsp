...
<html>
   ...
   <!-- 原本的CSS  <link rel="stylesheet" href="css/styles.css" type="text/css"/> -->  
	...
	<div id="divArea" class="btn-group" style="position: fixed; bottom:0px; right:5px;">
	    ...
	    <button id="controllBtn" type="button" class="btn btn-default"  
...
<!--大頭照-->  				<img src="<%=request.getContextPath()%>/frontend/DBPicReader/DBPicReader.do?id_no=${mem_no}&action=member" class="img-circle" width="50">
<!--姓名--> 					<span id="inputUserName">${mem_name}</span>
						
               
					<input type="hidden" name="mem_name" id="mem_name" value="${mem_name}">  <!--為了到時抓登入會員的會員名稱 要SHOW在聊天視窗內-->	
<!--搜尋Bar	                <div class="input-group mb-2 mr-sm-2 mb-sm-0" >
            ...
            <div class="panel panel-default">
                ...
                <div id="aaa" class="panel-collapse collapse in" role="tabpanel" aria-labelledby="panel1">
                    <ul class="list-group">
                    	<%--顯示會員好友列表--%>
						<c:forEach var="cflsit" items="${cfSvc.getOneMCF(mem_no) }">
							<%--判斷好友是否顯示--%>
            ...
            <div class="panel panel-default">
                ...
                <div id="bbb" class="panel-collapse collapse" role="tabpanel" aria-labelledby="panel2">
                    <ul class="list-group">
                    	<%--顯示會員群組列表--%>
            			<c:forEach var="cgVO" items="${cgiSvc.getOneChat_Group_Mem(mem_no) }">
...
<script>
    ...
    $(function(){
    	$("a[id^='addClass']").click(function () {
				...
				str+="        		</span>";str+="        	<input type=\"text\" placeholder=\"Type a message\" id=\"message\" name=\"message\" class=\"text-field\" onkeydown=\"if (event.keyCode == 13) sendMessage();\"/>";
				str+="         	<input type=\"submit\" id=\"sendMessage\" class=\"button\" value=\"送出\" onclick=\"sendMessage();\"/>";
				str+=" 	        <span style=\"vertical-align\: sub;\" class=\"uk-input-group-addon\">";
				
str+="			<form   name=\"form1\"   method=\"post\"   action=\"\"> ";
str+="        		<input   name=\"fileABC\"   type=\"file\"   style=\"display:none\">  ";	
str+=" 	      	  <a href=\"#\"  onclick=\"test1()\"><i class=\"fa fa-camera\"></i></a>";
str+="        	</form>";
